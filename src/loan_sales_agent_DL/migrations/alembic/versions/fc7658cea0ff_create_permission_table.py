...
Revision ID: fc7658cea0ff
Revises: 167b0ce3e22f
Create Date: 2026-07-08 14:23:03.573281
...
revision: str = 'fc7658cea0ff'
down_revision: Union[str, Sequence[str], None] = '167b0ce3e22f'
...
def upgrade() -> None:
    ...
    sa.Column('permission_key', sa.Enum('VIEW', 'EDIT', 'CREATE', 'DELETE', 'APPROVE', name='permission_enum'), nullable=False),
    ...
    op.create_index(op.f('ix_permission_permission_key'), 'permission', ['permission_key'], unique=False)
    op.create_table('rel_permission_customer',
    sa.Column('rel_permission_customer_id', sa.Integer(), nullable=False),
    ...
    sa.PrimaryKeyConstraint('rel_permission_customer_id')
    )
...
def downgrade() -> None:
    ...
    op.drop_table('rel_permission_customer')
    op.drop_index(op.f('ix_permission_permission_key'), table_name='permission')
